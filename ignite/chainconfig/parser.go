...
func validate(conf common.Config) error {
	if len(conf.ListAccounts()) == 0 {
		return &common.ValidationError{"at least 1 account is needed"}
	}
	if len(conf.ListValidators()) == 0 || conf.ListValidators()[0].Name == "" {
		return &common.ValidationError{"validator is required"}
	}
	return nil
}
